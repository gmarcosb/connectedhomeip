...
protected:
    ...
    DefaultSceneTableImpl(uint16_t maxScenesPerFabric, uint16_t maxScenesPerEndpoint) : FabricTableImpl(maxScenesPerFabric, maxScenesPerEndpoint)
    {}

    // Global scene count
    inline CHIP_ERROR SetEndpointSceneCount(const uint8_t & scene_count) {
      return SetEndpointEntryCount(scene_count);
    }
